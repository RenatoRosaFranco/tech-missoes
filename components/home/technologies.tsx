...
import type { Technology } from "@/lib/app-config-types";
...
export function Technologies({ technologies, mobilePositions, aroundTyped, copy }: TechnologiesProps) {
  ...
  function nodeStyle(index: number): CSSProperties {
    ...
    const [mobileX, mobileY] = mobilePositions[index] ?? [50, 50];
    return {
      "--mobile-x": `${mobileX}%`,
      "--mobile-y": `${mobileY}%`,
      "--node-x": `${orbit.x}%`,
    ...
    <section className="technologies section" id="tecnologias" aria-labelledby="technologies-title">
      ...
      <div className="container">
        <SectionHeading
          ...
          title={<>{copy.title}<br /><em><TypedLine inView strings={aroundTyped} /></em></>}
        >
        ...
        <div className="tech-universe" data-orbit-paused={orbitPaused}>
          <div className="tech-rotor">
            ...
            <svg className="tech-connections tech-connections-mobile" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
              {technologies.map((item, index) => (
                <line key={item.name} x1="50" y1="50" x2={mobilePositions[index]?.[0]} y2={mobilePositions[index]?.[1]} className={index === selected ? "active" : ""} />
              ))}
            </svg>
